...
describe('every address these answers hand out', () => {
  it('is a route this service has', async () => {
    ...
    await collect({ method: 'POST', url: `${project.api}/agents`, headers, payload: { handle: 'errors-loop' } });
    await collect({
      method: 'POST',
    ...
    await collect({ method: 'GET', url: `${project.api}/inbox`, headers });
    await collect({
      method: 'POST',
    ...
    await collect({ method: 'GET', url: '/v1/no-such-project/items' });
    await collect({ method: 'GET', url: '/there-is-nothing-here' });
    await collect({ method: 'POST', url: `${project.api}/items`, headers, payload: { slog: 'typo' } });
    ...
    const base = harness.config.baseUrl;
    const paths = new Set<string>();
    for (const body of said) {
      for (const [, path] of body.matchAll(new RegExp(`${base}(/[^"'\\s\\\\)]*)`, 'g'))) {
        // Trailing punctuation from a sentence that ends on an address.
        paths.add(path!.replace(/[.,;:]+$/, ''));
      }
    }
    assert.ok(paths.size >= 35, `the answers hand out addresses: found ${paths.size}`);
