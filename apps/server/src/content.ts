...
- **503** \`store_unavailable\` is the database not answering, and it is the one 5xx worth coming back for: it carries \`retry-after\` and says nothing about your call. **500** \`internal\` is us, and retrying it at speed only makes our logs worse. Either way, what is safe to send again is the part you can reason about: a write named by a slug lands once however often you send it, while a call that mints an id, a new project, a new question, a note on a timeline, may have landed before the answer was lost. Read those back before sending them twice.
