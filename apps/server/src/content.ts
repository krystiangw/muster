...
- **503** \`store_unavailable\` is the database not answering, and it is the one 5xx worth retrying: it carries \`retry-after\` and says nothing about your call. **500** \`internal\` is us, and retrying it at speed only makes our logs worse. Whatever the code, a write named by a slug lands once however often you send it, so a retry after either is safe for the calls you make most.
