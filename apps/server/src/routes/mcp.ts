    ...
    inputSchema: {
      ...
      properties: {
        ...
        prefix: {
          type: 'string',
          description:
            `Slug starts with this. Boards name their cards \`area:thing\`, so prefix "ops:" is everything in one area. Anchored, so it bounds the read to its own stretch of the slug index, which q cannot do: a substring over two fields is not answerable from an index. The two combine, and this is the half that makes the search cheaper. The filters that bound what a search reads are ${SEARCH_NARROWING}; label= narrows the answer and not the read, because labels carry no index here. board_facets lists the namespaces this board has.`,
        },
        stale: { type: 'boolean' },
        claimed: {
          type: 'boolean',
